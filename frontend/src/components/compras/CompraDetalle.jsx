    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl max-h-screen overflow-y-auto p-6">
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            <h3 className="text-xl font-bold text-gray-800">Compra #{compra.id}</h3>
            <p className="text-gray-500 text-sm">{compra.fecha} — {compra.proveedor_nombre}</p>
          </div>
            ...
            >
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <p className="text-sm text-gray-600 mt-1">
                    {d.kilos} kg
                    {d.precio_kilo && ` × $${Number(d.precio_kilo).toLocaleString('es-CO')}/kg`}
                  </p>
                </div>
              ...
              {d.es_deposito && d.liquidaciones?.length > 0 && (
                <div className="mt-3 border-t border-yellow-200 pt-3">
                  <p className="text-xs font-semibold text-gray-500 mb-2">Liquidaciones:</p>
                  {d.liquidaciones.map(l => (
                    <div key={l.id} className="flex justify-between text-xs text-gray-600">
                      <span>{l.fecha} — {l.kilos} kg × ${Number(l.precio_kilo).toLocaleString('es-CO')}/kg</span>
                      <span className="font-semibold">${Number(l.subtotal).toLocaleString('es-CO')}</span>
                    </div>
                  ))}
                </div>
              )}
        ...
        <div className="mt-6 bg-gray-50 rounded-lg p-4 space-y-2">
          <div className="flex justify-between text-sm text-gray-600">
            ...
            <span className="font-semibold">${Number(compra.total).toLocaleString('es-CO')}</span>
          </div>
          {Number(compra.total_deposito_pendiente) > 0 && (
            <div className="flex justify-between text-sm text-yellow-600">
              <span>Depósito pendiente:</span>
              <span className="font-semibold">${Number(compra.total_deposito_pendiente).toLocaleString('es-CO')}</span>
            </div>
          )}
        </div>
