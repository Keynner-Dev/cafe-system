...
export default function VentaModal({ onClose, onSaved }) {
  const [form, setForm] = useState({ cliente: "", fecha: hoy, nota: "" });
  ...
  useEffect(() => {
    getTerceros().then((res) =>
      setClientes(
        res.data.filter((t) => t.tipo === "cliente" || t.tipo === "ambos"),
      ),
    );
  ...
  const handleDetalleChange = (index, e) => {
    ...
    if (name === "tipo_cafe") {
      const precioHoy = preciosHoy.find(
        (p) => String(p.tipo_cafe) === String(value),
      );
  ...
  const totalVenta = detalles.reduce(
    (acc, d) => acc + (Number(d.kilos) * Number(d.precio_kilo) || 0),
    0,
  );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data?.stock) {
        setError(data.stock.join(" | "));
      } else {
        setError("Error al guardar la venta. Verifica los datos.");
      }
    } finally {
      setLoading(false);
    }
                ...
                >
                  ...
                  <div className="flex justify-between items-center mt-3">
                    <span className="text-xs text-gray-400">
                      ...
                      preciosHoy.find(
                        (p) =>
                          String(p.tipo_cafe) === String(detalle.tipo_cafe),
                      )
                        ? `Precio hoy: $${Number(preciosHoy.find((p) => String(p.tipo_cafe) === String(detalle.tipo_cafe)).precio).toLocaleString("es-CO")}/kg`
                        : "Sin precio registrado hoy"}
                    </span>
                    <div className="flex items-center gap-3">
                      {detalle.kilos && detalle.precio_kilo && (
                        <span className="text-sm font-semibold text-gray-700">
                          Subtotal: $
                          {(
                            Number(detalle.kilos) * Number(detalle.precio_kilo)
